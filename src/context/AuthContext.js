...
export const AuthProvider = ({ children }) => {
  ...
  const register = async (userData) => {
    try {
      ...
      // Create a NextAuth session so the backendToken is available everywhere.
      // This calls the backend login endpoint which returns the token for NextAuth JWT.
      ...
      // Wait for session to update, then verify
  ...
  const login = async (credentials) => {
    try {
      ...
      // Give enough delay for session to update and interceptors to refresh
      await new Promise((r) => setTimeout(r, 500));
      
      // Try to get the token from the updated session and store in localStorage
      try {
        const sessionRes = await fetch('/api/auth/session');
        const sessionData = await sessionRes.json();
        if (sessionData?.backendToken && typeof window !== 'undefined') {
          window.localStorage.setItem('auth_token', sessionData.backendToken);
        }
      } catch {}
