...
const ServerCheck = ({ children }) => {
  ...
  const maxRetries = 10; // More retries for Render cold starts (can take 30-60 seconds)
    ...
    const checkServer = async () => {
      try {
        ...
        if (retryCount < maxRetries) {
          ...
          // Retry after 3 seconds (total wait ~30 seconds for cold start)
          setTimeout(() => {
            checkServer();
          }, 3000);
        } else {
      ...
      <div className="min-h-screen bg-zinc-950 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-zinc-900/60 border border-red-500/20 rounded-xl p-6 text-center">
          ...
          <p className="text-gray-400 mb-4">
            Unable to connect to the backend server. Please make sure the server
            is running.
          </p>
          <button
            onClick={() => {
